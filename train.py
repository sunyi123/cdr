...
parser.add_argument('--load', dest='load',  default=False, help='Load pretrained model.')
...
if not opt['load']:
    ...
    f_auc, dev_f, dev_p, dev_r, maxac, maxj, _, _, _, _ = evaluate.f_compute(gold_answer,predictions_list)
    _, intra_dev_f, intra_dev_p, intra_dev_r, _, _, _, _, _, _ = evaluate.f_compute(intra_answer, intra_predictions)
    _, inter_dev_f, inter_dev_p, inter_dev_r, _, _, _, _, _, _ = evaluate.f_compute(inter_answer, inter_predictions)

    f_test_pred = open('./output_test_prediction.txt', "w")
    for i in range(len(predictions_list)):
        if predictions_list[i] >= maxj:
            f_test_pred.write('\t'.join([doc_id_list[i], 'CID', mesh_id_list[i][0], mesh_id_list[i][1]], distance_list[i]) + '\n')
    f_test_pred.close()
...
for epoch in range(1, opt['num_epoch']+1):
    ...
    dev_score = dev_f
    file_logger.log("{}\t{:.6f}\t{:.6f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}".format(epoch, train_loss, dev_loss, dev_p, dev_r, dev_f, max([dev_score] + dev_score_history)))
