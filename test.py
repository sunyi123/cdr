...
parser.add_argument('--bert_dim', type=int, default=768, help='elmo embedding dimension.')
parser.add_argument('--hidden_dim', type=int, default=500, help='GCN and MLP hidden state size.')
parser.add_argument('--use_elmo', type=bool, default=1, help='Use elmo or not')
parser.add_argument('--use_bert', type=bool, default=1, help='Use bert or not')
...
parser.add_argument('--heads', type=int, default=5, help='num of heads for multi-head attention')

parser.add_argument('--lr', type=float, default=0.00001, help='Applies to sgd and adagrad.')
parser.add_argument('--lr_decay', type=float, default=0.9, help='Learning rate decay rate.')
parser.add_argument('--decay_epoch', type=int, default=1, help='Decay learning rate after this epoch.')
...
parser.add_argument('--id', type=str, default='11', help='Model ID under which to save models.')
...
parser.add_argument('--model_file', type=str, default='./saved_models/11/best_model.pt',help='Filename of the pretrained model.')
...
parser.add_argument('-meshembedding_dim', dest="meshembedding_dim",  default=1024,  help='meshembedding_dim')
parser.add_argument('-wordembedding_dim', dest="wordembedding_dim",  default=200,  help='wordembedding_dim')

parser.add_argument('-meshembedding_file', dest="meshembedding_file", default='./dataset/mesh/transE_1024.txt', help='meshembedding')
parser.add_argument('-meshdic_file', dest="meshdic_file", default='./entity_dic.pkl.gz', help='meshdic_file')
...
if not opt['load']:
    ...
    f_auc, dev_f, dev_p, dev_r, maxac, maxj, TP, FP, TN, FN = evaluate.f_compute(gold_answer,predictions_list)
    #_, intra_dev_f, intra_dev_p, intra_dev_r, _, _, intra_TP, intra_FP, intra_TN, intra_FN = evaluate.f_compute(intra_answer, intra_predictions)
    #_, inter_dev_f, inter_dev_p, inter_dev_r, _, _, inter_TP, inter_FP, inter_TN, inter_FN = evaluate.f_compute(inter_answer, inter_predictions)
    intra_predictions = []
    intra_pre = []
    intra_answer = []
    inter_predictions = []
    ...
    for pre in intra_predictions:
        if pre > maxj:
            intra_pre.append(1)
        else:
            intra_pre.append(0)
            
    for pre in inter_predictions:
        if pre > maxj:
            inter_pre.append(1.0)
        else:
            inter_pre.append(0.)
            
    intra_dev_p, intra_dev_r, intra_dev_f, intra_TP, intra_FP, intra_TN, intra_FN = evaluate.f_score(intra_pre, intra_answer)
    inter_dev_p, inter_dev_r, inter_dev_f, inter_TP, inter_FP, inter_TN, inter_FN = evaluate.f_score(inter_pre, inter_answer)
          
    

    f_test_pred = open('./output_test_prediction_'+opt['id']+'.txt', "w")
    ...
    print('document level TP, FP, TN, FN is: ', TP, FP, TN, FN)
    print('Test results for intra-sentence level: Prec:{} | Rec:{} | F1:{} '.format(intra_dev_p, intra_dev_r, intra_dev_f))
    print('intra-sentence level TP, FP, TN, FN is: ', intra_TP, intra_FP, intra_TN, intra_FN)
    print('Test results for inter-sentence level: Prec:{} | Rec:{} | F1:{} '.format(inter_dev_p, inter_dev_r, inter_dev_f))
    print('inter-sentence level TP, FP, TN, FN is: ', inter_TP, inter_FP, inter_TN, inter_FN)
